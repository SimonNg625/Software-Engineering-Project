...
import sportapp.screen.SellEquipmentScreen;
import sportapp.ui.SellEquipmentUI;

public class TestSellEquipment {	
	private User user;
	private Scanner scanner;
	private ByteArrayInputStream inContent;

	private SellEquipmentScreen sellEquipmentScreen;
	...
	public void setUp() throws Exception {
		sellEquipmentUI = new SellEquipmentUI(null, null);
	...
	public void prepareEquipmentType() {
		...
		testTypes.add(new EquipmentType("ET-007", "Badminton Shuttlecock (6 pcs)", "BBALL6", "Badminton", 35));
		testTypes.add(new EquipmentType("ET-009", "Table Tennis Ball (3 pcs)", "TBALL1", "TableTennis", 15));
	...
	public void prepareEquipment() {
		int quantity = 5;
		for (EquipmentType type: testEquipmentTypeManager.getSellableTypes()) {
	...
	public void prepareEquipmentRecord(FacilityBookRecord facilityRecord, int quantity) {
		...
		EquipmentType testEquipType = testEquipmentTypeManager.getSellableTypesBySportType(sportType).getFirst();
		ArrayList<Equipment> testEquips = testEquipmentManager.getSellableEquipmentByType(testEquipType);
		
		testEquipmentBookManager.addBookRecord(new EquipmentBookRecord(facilityRecord, testEquips, user));
	}
	...
	public void test_display_noEquipmentType() {
		...
		sellEquipmentScreen = new SellEquipmentScreen(testFacilityBookManager.getBookingRecords().get(0));
        ...
        assertEquals(expected, sellEquipmentScreen.display(scanner, user));
	}
	
	@Test
	public void test_display_noEquipmentType_wrongSelectRoute() {
		prepareFacilityRecord();
	...
	public void test_display_inputEquipmentType_notB_notfound() {
		...
		sellEquipmentScreen = new SellEquipmentScreen(testFacilityBookManager.getBookingRecords().get(1));
	...
	@Test
	public void test_inputEquipmentType_notB_incorrectSportType() {
		prepareFacilityRecord();
		prepareEquipmentType();
		prepareEquipment();
	...
	@Test
	public void test_display_inputQuantity_above50() {
		prepareFacilityRecord();
		
		FacilityBookRecord testFacilityBookRecord = testFacilityBookManager.getBookingRecords().get(1);
		sellEquipmentScreen = new SellEquipmentScreen(testFacilityBookRecord);
...
//		prepareEquipmentRecord(testFacilityBookRecord, 3);
	...
	@Test
	public void test_display_sellEquipment_back() {
		prepareFacilityRecord();
...
//		prepareEquipmentRecord(testFacilityBookRecord, 3);
        ...
        Route expected = Route.BACK;
        assertEquals(expected, sellEquipmentScreen.display(scanner, user));
        assertEquals(true, matchEquipmentRecord("ET-007", testFacilityBookRecord, 3));
	}

	@Test
	public void test_display_sellEquipment_home() {
		prepareFacilityRecord();
